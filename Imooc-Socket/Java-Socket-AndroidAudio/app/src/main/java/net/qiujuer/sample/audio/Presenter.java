...
import clink.box.StreamDirectSendPacket;
...
import clink.utils.plugin.CircularByteBuffer;
...
public class Presenter implements AppContract.Presenter, ServerNamedConnector.ConnectorStatusListener {
    ...
    // 音频录制缓冲区，录制的声音将缓存到当前缓冲区等待被网络读取发送【缓冲区不宜过大，否则声音延迟】
    private final CircularByteBuffer mAudioRecordBuffer = new CircularByteBuffer(128, true);

    // 音频播放缓冲期，网络接收的语音将存储到当前缓冲区等待被播放器读取播放【缓冲区大一点，尽可能多接收数据】
    ...
    private final ServerNamedConnector.MessageArrivedListener mMessageArrivedListener = info -> {
        switch (info.getKey()) {
